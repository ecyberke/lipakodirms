...
<div class="content container-fluid">
    ...
    <!-- Content Starts -->
    <div class="card">
        <form action="{{route('report.rent_report')}}" method="get">
            @csrf
                ...
                <div class="container-fluid">
                  ...
                  <div class="row table-responsive   ">
                            ...
                            <tbody>
                               @forelse($entries as $row)
                                <tr>
                                    ...
                                    <td>{{ number_format($row->total_rent_collected, 2) }}</td>
                                    <td>{{ number_format($row->management_fee_percentage, 2) }}%</td>
                                    <td>{{ $row->bill_names ?? '-' }}</td>
                                    <td>{{ number_format($row->total_bill_amount_paid, 2) }}</td>
                                    <td>{{ number_format($row->net_rent_remitted, 2) }}</td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="7" class="text-center">No data available for the selected month.</td>
                                </tr>
                            @endforelse
                              
                            </tbody>
                            @php
                                $totalRent = $entries->sum('total_rent_collected');
                                $totalBills = $entries->sum('total_bill_amount_paid');
                                $totalNet = $entries->sum('net_rent_remitted');
                            @endphp
                    ...
                    <div class="row">
                            ...
                            <div class="col-md-4 text-center" >
                                <a  href="{{\Request::fullUrl()}}&download=yes" target="_blank" class="m-3 btn btn-success waves-effect waves-light">Download Report</a>
                            </div>
