...
class LandLordsController extends Controller
{
    ...
    public function delete($id)
    {
        ...
        $apartments = Apartment::where('landlord_id', $id)->get();
         ...
         foreach ($apartments as $apartment) {
            $house = House::where('apartment_id', $apartment->id)->get();
        ...
        foreach ($apartments as $apartment) {
           $house = House::where('apartment_id', $apartment->id)->forceDelete();
           $invoices = Invoice::where('apartment_id', $apartment->id)->forceDelete();
           $deposit = Deposit ::where('apartment_id', $apartment->id)->forceDelete();;
           $service_request = ServiceRequests ::where('apartment_id', $apartment->id)->forceDelete();
           $pay_owner = PayOwners::where('apartment_id', $apartment->id)->forceDelete();
        }
        $apartments = Apartment::where('landlord_id', $id)->forceDelete();
        ...
        return back()->with('success','Property Owner has been permanently deleted from system with all information linked to it.');
         ...
         DB::rollback();
         return back()->with('error', 'System error deleting the property owner, please contact the System Administrator.');
}
