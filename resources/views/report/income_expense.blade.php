...
<div class="content container-fluid">
    ...
    <!-- Content Starts -->
    <div class="card">
          <form action="{{route('report.agency_income_expense_report')}}" method="get">
            @csrf
                ...
                <div class="title text-center">
                    ...
                    <div class=" ">
                        ...
                        <!--<table class="table dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 500px; width: 100%;">-->
                            ...
                            <tbody>
                                @foreach($properties_paid_rent as $property)
                              <tr>
                               
                                <td >{{$property['apartment_name']}}</td>
                                <td class="text-right">{{ number_format($property['total_paid_in'], 2) }}</td>
                                <td class="text-right">{{ number_format(($property['total_paid_in']*($property['commission']/100)), 2) }}</td>
                              </tr>
                              @endforeach
        ...
        <div>
             <table id="agency-status-table2"  style="width: 100%;">
                        <!--<table class="table dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 500px; width: 100%;">-->
                            ...
                            <tbody>
                                @foreach($agency_income as $inc)
                              <tr>
                               
                                <td >{{ \Carbon\Carbon::parse($inc->created_at)->format('d M Y') }}</td>
                                <td >{{ $inc->full_name }}</td>
                                  <td >{{ $inc->phone }}</td>
                                <td >{{ number_format($inc->fee, 2) }}</td>
                              </tr>
                              @endforeach
                ...
                <div class="title text-center">
                    ...
                    <div >
                        ...
                        <!--<table class="table dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 500px; width: 100%;">-->
                            ...
                            <tbody>
                                @foreach($agency_bills as $bill)
                              <tr>
                               
                                <td >{{ \Carbon\Carbon::parse($bill->created_at)->format('d M Y') }}</td>
                                <td >{{ $bill->description }}</td>
                                <td >{{ number_format($bill->total_owned, 2) }}</td>
                              </tr>
                              @endforeach
                        ...
                        <table class="table dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 500px; width: 100%;">
                             <thead>
                              ...
                              <tr>
                                <th colspan="4"> </th>
                                <th >Agency Income</th>
                                <th >{{ number_format($total_agency_commission, 2) }}</th>
                              </tr>
                              <tr>
                                <th colspan="4"> </th>
                                <th >Agency CRM Income</th>
                                <th >{{ number_format($total_CRM, 2) }}</th>
                              </tr>
                               <tr>
                                <th colspan="4"> </th>
                                <th >Agency Expense</th>
                                <th >{{ number_format($total_agency_bills, 2) }}</th>
                              </tr>
                              <tr>
                                <th colspan="4"> </th>
                                <th >Agency Total Amount</th>
                                <th >{{ number_format(($total_agency_commission + $total_CRM - $total_agency_bills), 2) }}</th>
                              </tr>
                            </tbody>
                          </table>
                    </div>
