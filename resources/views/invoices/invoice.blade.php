...
<div class="page-header">
    ...
    <div class="page-rightheader ml-auto d-lg-flex d-none">
        <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="{{ route('home')}}" class="d-flex">
    ...
    <div class="col-md-12">
        ...
        @endif
                <div class="col-sm-6 col-md-4 col-xl-3">
                    ...
                    <button type="button" class="btn btn-success d-grid mb-3" data-bs-toggle="modal" data-bs-target="#stkPushModal">
                        Initiate Payment Prompt
                    </button>
                    
                    <!-- Manual Payment Modal Trigger Button - FIXED -->
                    <!--<button type="button" class="btn btn-info d-grid mb-3" data-bs-toggle="modal" data-bs-target="#manualPaymentModal">-->
                ...
                <div class="row pt-4">
                        ...
                        <address>
                            <li>House:  
                                ...
                                @elseif($invoice->house_name == null)
                                    <span class="text-success">{{ $invoice->house->house_no ?? 'N/A' }}</span> -  
                                    Rent Paid after every <strong>{{ $invoice->house->rent_const ?? 'N/A' }} month(s)</strong>
                            ...
                            <li>Property: 
                                ...
                                @elseif($invoice->apartment_id > 0)
                                    <span class="text-success">{{ $invoice->apartment->name ?? 'N/A' }}</span>
                                @elseif($invoice->apartment_id == 0)
                    ...
                    <div class="col-lg-6 text-right">
                        ...
                        <address>
                            ...
                            Account Number: 
                            @if($invoice->tenant_id > 0)
                                {{ $invoice->tenant->account_number ?? 'N/A' }}
    ...
    <table class="table table-bordered table-hover text-nowrap">
        ...
        @if($invoice->type == 'Monthly Rent' || $invoice->type == 'rent and deposit' || $invoice->type == null || $invoice->type == 'House Viewing')
            ...
            <tr>
                ...
                <td class="text-right">Ksh {{ number_format($invoice->service_charge)}}</td>
            </tr>
            @endif
              @if($invoice->other_charges > 0)
            <tr>
                <td class="d-none d-sm-table-cell">Other Charges </td>
                <td class="text-right">Ksh {{ number_format($invoice->other_charges)}}</td>
            </tr>
            @endif
              @if($invoice->statutory_fee > 0)
            <tr>
                <td class="d-none d-sm-table-cell">Statutory Fee </td>
                <td class="text-right">Ksh {{ number_format($invoice->statutory_fee)}}</td>
            </tr>
            @endif
        ...
        <tr>
            ...
            <td class="text-right text-success text-bold">Ksh {{ number_format($invoice->paid_in ) }}</td>
        </tr>
        <tr>
        ...
        <tr>
            <td colspan="5" class="text-right">
                ...
                @if($invoice->balance > 0)
                    <div class="alert alert-info">
                        ...
                        <p class="mb-0"><strong>Option 2: Pay via Lipa na Mpesa (Paybill)</strong><br>
                           ...
                           Account Number: 
                           @if($invoice->tenant_id != null && $invoice->tenant)
                               {{ $invoice->tenant->account_number }}
                           @else
                               {{ $invoice->tenant_acc ?? $invoice->invoice_number }}
                           @endif
                           <br>
                ...
                <p class="text-muted mt-3">
                    <i class="fa fa-calendar"></i> All rent payments are due on or before 5th {{ $invoice->rent_month }}
                </p>
            </div>
        </div>
    </div>
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="stkPushModalLabel">
                    ...
                    Account #{{ $invoice->tenant->account_number }} - STK Push Payment
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            
            <form action="{{ route('mpesa.stk.push', $invoice->id) }}" method="POST">
                @csrf
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group mb-3">
                        ...
                        <small class="text-muted">
                            ...
                            Min: 1 | Max: {{ number_format($invoice->balance) }} (Full balance: {{ number_format($invoice->balance) }})
                        </small>
                    </div>
                    ...
                    <div class="form-group mb-3">
                        ...
                        <div class="input-group">
                            ...
                            <input type="text" 
                                   ...
                                   id="phone" 
                                   name="phone" 
                                   value="{{ old('phone', $invoice->tenant_phone ?? ($invoice->tenant->phone ?? '')) }}" 
                    ...
                    <div class="bg-light p-3 rounded mb-3">
                        <div class="d-flex justify-content-between mb-2">
                            ...
                            <span class="fw-bold">{{ $invoice->tenant->account_number }}</span>
                        </div>
                        <div class="d-flex justify-content-between mb-2">
                            <span>Total Balance:</span>
                            <span class="fw-bold text-danger">Ksh {{ number_format($invoice->balance) }}</span>
                        </div>
                        <div class="d-flex justify-content-between pt-2 border-top">
                            <span class="h6 mb-0">Amount to be paid:</span>
                            <span class="h6 mb-0 text-primary" id="displayAmount">Ksh {{ number_format($invoice->balance) }}</span>
                        </div>
                    </div>
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" class="btn btn-primary" id="submitBtn">
                        <i class="fa fa-mobile"></i> Initiate Prompt Ksh. <span id="submitAmount">{{ number_format($invoice->balance) }}</span>
                    </button>
                </div>
            </form>
        </div>
                ...
                <div class="card bg-light">
                    <div class="card-body">
                        ...
                        <ol class="list-group list-group-numbered mb-3">
                            ...
                            <li class="list-group-item d-flex justify-content-between align-items-center">
                                Enter Amount
                                <span class="badge bg-danger rounded-pill fs-6">Ksh {{ number_format($invoice->balance) }}</span>
                            </li>
